...
if __name__ == '__main__':
    ...
    net = ARCH_REGISTRY.get('CodeFormer')(dim_embd=512, codebook_size=512, n_head=8, n_layers=9, connect_list=['32', '64', '128']).to(device)
    ...
    for i, img_path in enumerate(input_img_list):
        ...
        try:
            with torch.no_grad():
                ...
                save_face_in_org = tensor2img(input_face, rgb2bgr=True, min_max=(-1, 1))
                #################################################

                # w is fixed to 1, adain=False for inpainting
                output_face = net(input_face, w=1, adain=False)[0]
                output_face_org = output_face
                output_face = (1-mask)*input_face + mask*output_face
                save_face = tensor2img(output_face, rgb2bgr=True, min_max=(-1, 1))

                save_face_out_org = tensor2img(output_face_org, rgb2bgr=True, min_max=(-1, 1))
                save_face_in_mask = tensor2img((1-mask)*input_face, rgb2bgr=True, min_max=(-1, 1))
                save_face_out_mask = tensor2img(mask*output_face, rgb2bgr=True, min_max=(-1, 1))
        ...
        save_face_in_org = save_face_in_org.astype('uint8')
        save_face_in_mask = save_face_in_mask.astype('uint8')
        save_face_out_org = save_face_out_org.astype('uint8')
        save_face_out_mask = save_face_out_mask.astype('uint8')

        # save face
        if args.suffix is not None:
            basename = f'{basename}_{args.suffix}'

        save_restore_path = os.path.join(result_root, f'{basename}' + '_in' + '.png')
        imwrite(save_face_in_org, save_restore_path)

        # save_restore_path = os.path.join(result_root, f'{basename}' + 'mask' + '.png')
        # imwrite(mask_img, save_restore_path)

        save_restore_path = os.path.join(result_root, f'{basename}' + '_in+mask' + '.png')
        imwrite(save_face_in_mask, save_restore_path)

        save_restore_path = os.path.join(result_root, f'{basename}' + '_out+org' + '.png')
        imwrite(save_face_out_org, save_restore_path)

        save_restore_path = os.path.join(result_root, f'{basename}' + '_out+mask' + '.png')
        imwrite(save_face_out_mask, save_restore_path)

        save_restore_path = os.path.join(result_root, f'{basename}'+ '_result'+ '.png')
